...
def perform_test(test_loader, model, preprocess,
    ...
    for i, data in enumerate(test_loader):
        ...
        # print("Processing {}: shape is {}".format(input_file, video.shape))
        # (features_length, T/features_length, 224, 224, 3)
        ...
        if video_shape_len == 5:
            ...
            for _, (min_ind, max_ind, fast_clip) in enumerate(clip_loader):
                # print('fast_clip.shape:', fast_clip.shape)
                # B T H W C. B = 16
...
def main():
    ...
    preprocess = Preprocessing(
        ...
        features_length=args.features_length
    )
    if args.dataflow:
        readvideo = ReadVideo(
            preprocess,
